...
r1 = requests.get('https://bugzilla.kernel.org/rest/bug?token=209397-5fw5xknGJeOI9WchYhq6gp&format=json&status=NEW&status=NEEDINFO&status=REOPENED&status=ASSIGNED&include_fields=creation_time,id,status,assigned_to')
...
for bug in r1.json()['bugs']:
    ...
    result.append(data)
workbook=xlsxwriter.Workbook('OpenBugsInformation.xlsx')
...
worksheet.write('G1','CreationTime',bold)
row,col=1,0
for id,realname,name,uid,status,assignedto,creationtime in result:
    ...
    worksheet.write(row,col+6,creationtime)
    row+=1
workbook.close()